...
namespace SurfBoardApp.Areas.api
{
    ...
    public class BoardController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetBoardNames()
        {
            try
            {
                var boardnames = await _context.Board
                    .Select(b => b.Name)
                    .ToListAsync();

                if (boardnames != null && boardnames.Count > 0)
                {
        ...
        // Create a new board
        [HttpPost]
        public async Task<IActionResult> CreateBoard([FromBody] BoardModel board)
        {
            try
            {
                if (board == null)
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<BoardModel>> GetBoard(int id)
        {
            var board = await _context.Board.FindAsync(id);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateBoard(int id, BoardModel board)
        {
            if (id != board.Id)
            {
